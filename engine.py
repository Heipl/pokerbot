...
def _safe_subprocess_timeout(timeout):
    """Return a communicate() timeout that won't overflow selector/poll.

    On Python 3.12, selectors.PollSelector ultimately calls poll.poll(ms)
    where ms must fit in a signed 32-bit int. Extremely large timeouts can
...
class Player():
            ...
            try:
                ...
                with server_socket:
                    ...
                    def enqueue_output(out, queue):
                        try:
                            for line in out:
                                # Always retain output for per-bot log files.
                                queue.put(line)

                                # Also echo to the engine console so you can see
                                # bot-side print() progress (training heartbeats, etc.).
                                try:
    ...
    def stop(self):
        ...
        if self.bot_subprocess is not None:
            try:
                outs, _ = self.bot_subprocess.communicate(timeout=_safe_subprocess_timeout(CONNECT_TIMEOUT))
