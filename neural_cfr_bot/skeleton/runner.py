        ...
        for packet in self.receive():
            for clause in packet:
                if clause[0] == 'T':
                    ...
                    # Advance the round counter. It was set to 1 at startup and every
                    # later GameState copied round_num verbatim, so bots saw round 1
                    # for the whole 1000-hand match. Anything keyed to hand number was
                    # therefore frozen -- in neural_cfr_bot that silently disabled the
                    # Deep MCCFR training trigger (`round_num % 5 == 0` is never true
                    # when round_num is always 1), and it breaks any per-hand annealing
                    # or opponent model that assumes it can tell hand 1 from hand 900.
                    # This clause is the end of a round, so it is the right place.
